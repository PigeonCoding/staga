...
//   // int end = 0;
//   for (int k = 0; (size_t)k < array_length(instr) && (size_t)k <=
//   current_instr; k++) {
...
//   for (int k = 0; (size_t)k <= new_current_instr; k++) {
...
#define tok_not_end tok->count != tok->og->length

string_view get_next_token(tokenizer *tok) {
  string_view s = {0};

  while (is_whitespace(pseudo_str(*tok->og)[tok->count]) && tok_not_end) {
    tok->count++;
  }
  ...
  if (is_numerical(pseudo_str(*tok->og)[tok->count])) {
    ...
    s.length++;
    while (is_numerical(pseudo_str(*tok->og)[tok->count]) && tok_not_end) {
      s.length++;
      tok->count++;
    }
    s.length--;
  ...
  if (is_alphabetical(pseudo_str(*tok->og)[tok->count])) {
    s.base_pointer = tok->og->base_pointer + tok->count * char_size;
    s.length++;
    while (is_alphabetical(pseudo_str(*tok->og)[tok->count]) && tok_not_end) {
      s.length++;
      tok->count++;
    }
    s.length--;
